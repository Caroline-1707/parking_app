...
from root.models import Client, ClientParking, Parking
...
def test_create_parking(client, db_session):
    data = {"address": "456 Street St", "opened": True, "count_places": 20}
...
def test_enter_parking(client, sample_client, sample_parking, db_session):
    ...
    data = {"client_id": sample_client.id, "parking_id": sample_parking.id}
...
def test_exit_parking(
    client, sample_client, sample_parking, sample_client_parking, db_session
):
    data = {"client_id": sample_client.id, "parking_id": sample_parking.id}
...
def test_enter_closed_parking(client, sample_client, db_session):
    ...
    db_session.commit()
    data = {"client_id": sample_client.id, "parking_id": parking.id}
    response = client.post("/client_parkings", json=data)
...
def test_exit_without_card(client, db_session):
    ...
    db_session.commit()
    data = {"client_id": client_no_card.id, "parking_id": parking.id}
    response = client.delete("/client_parkings", json=data)
    assert response.status_code == 400
